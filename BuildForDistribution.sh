...
for i in {1..4}; do
  ...
  cmake -B $BUILD_DIR -DX86_64_VERSION=$i -DCMAKE_BUILD_TYPE=$1 $2
  echo "---------- BUILDING x86_64 v$i ----------"
  cmake --build $BUILD_DIR --target game $3
done
cp $BUILD_DIR/discord_game_sdk.so $BUILD_DIR/out/bin
clean_build_dir

echo "---------- CONFIGURING Launcher ----------"
cmake -B $BUILD_DIR -DX86_64_VERSION=$i -DSTANDALONE_LAUNCHER=ON -DCMAKE_BUILD_TYPE=$1 $2
echo "---------- BUILDING Launcher ----------"
cmake --build $BUILD_DIR --target launcher $3
clean_build_dir
