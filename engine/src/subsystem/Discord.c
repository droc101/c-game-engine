...
void DiscordUpdateRPC()
{
	...
	{
		case IN_GAME:
			snprintf(activity.assets.large_text, 128, "Playing %s", GetState()->mapName);
			snprintf(activity.details, 128, "Playing %s", GetState()->mapName);
			snprintf(activity.assets.large_image, 128, "map_%s", GetState()->mapName);
			break;
		case PAUSED:
			snprintf(activity.assets.large_text, 128, "Playing %s", GetState()->mapName);
