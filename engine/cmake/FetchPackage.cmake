...
macro(import_steamworks_sdk)
    message(STATUS "Importing Steamworks SDK...")

    if (NOT EXISTS "${STEAMWORKS_SDK_PATH}")
        message(FATAL_ERROR "Steamworks SDK path does not exist!")
    else ()
        add_library(steamworks SHARED IMPORTED)
        if (WIN32)
            if (x86_64)
                file(COPY_FILE ${STEAMWORKS_SDK_PATH}/redistributable_bin/win64/steam_api64.dll ${CMAKE_BINARY_DIR}/steam_api64.dll)
                set_target_properties(steamworks PROPERTIES IMPORTED_LOCATION steam_api64.dll IMPORTED_IMPLIB ${STEAMWORKS_SDK_PATH}/redistributable_bin/win64/steam_api64.lib)
            else ()
                message(FATAL_ERROR "Windows builds with Steamworks support are only available on x86_64")
            endif ()
        else ()
            if (x86_64)
                file(COPY_FILE ${STEAMWORKS_SDK_PATH}/redistributable_bin/linux64/libsteam_api.so ${CMAKE_BINARY_DIR}/libsteam_api.so)
                set_target_properties(steamworks PROPERTIES IMPORTED_LOCATION libsteam_api.so)
            else ()
                file(COPY_FILE ${STEAMWORKS_SDK_PATH}/redistributable_bin/linuxarm64/libsteam_api.so ${CMAKE_BINARY_DIR}/libsteam_api.so)
                set_target_properties(steamworks PROPERTIES IMPORTED_LOCATION libsteam_api.so)
            endif ()
        endif ()
